	...
	<div class="row" id="content_row">
		...
		<% } %>
			...
			<form name="frm1" id="frm1" action="<%=request.getContextPath() %>/CartToInsertSaleCtrl" method="post" onsubmit="return payCheck(this)">
				...
				<table class="table">
					<tbody>
						<tr>
							...
							<button type="button" id="post_btn" onclick="findAddr()" class="btn btn-outline-warning" style="margin-bottom:36px;">우편번호 검색</button>
							</td>
						</tr>
						<tr>
							<th>받는 사람 연락처</th>
				...
				<div class="btn-group" style="margin-bottom:36px;">
				<% if(vo.getAmount()!=0) { %>
				<input type="submit" class="btn btn-outline-warning" value="제품 구매">
				<% } %>
				</div>
			</form>
			<script>
			...
			<div class="btn-group">
				<a href="<%=request.getContextPath() %>/GetProductListCtrl" class="btn btn-outline-warning">목록으로</a>
				<% if(sid.equals("admin")) { %>
				<a href="<%=request.getContextPath() %>/DeleteProductCtrl?proNo=<%=vo.getProNo() %>" class="btn btn-outline-warning">제품 삭제</a>
				<a href="<%=request.getContextPath() %>/UpdateProductCtrl?proNo=<%=vo.getProNo() %>" class="btn btn-outline-warning">제품 정보 수정</a>
